...
# directions. The figures are regenerated by `scripts/retraction_accuracy.jl`, which measures the
# same matrix this file does -- it is `svd_matrix.jl` for both. The spreads are over seeds 1..8 at a
# 20_000-iteration cap, which is what "cap" in the last column means: those two combinations do not
# converge on two of the eight, and that is open issue A1b.
#
...
#
# Three `Cayley` lower bounds in the table above were stale before this pass and are corrected here:
