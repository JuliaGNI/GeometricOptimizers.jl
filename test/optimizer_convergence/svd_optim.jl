...
# whole sweep is `2.5e-7`, so the qualifier is gone and the factor is 40. `g_converged` is still
...
#     _BFGS  Backtracking(expand)     95 /   118        2_441 /  3_031     104..162 /  96..154
#     _BFGS  Backtracking            136 /   136        3_457 /  3_456     146..197 / 114..187
#     _BFGS  Bisection               133 /   114       78_658 / 67_030       81..125 / 102..137
#     _BFGS  StrongWolfe(c₂=0.1)     135 /   135        7_893 /  7_880      100..143 / 107..157
#     _BFGS  Quadratic               120 /    98       15_422 / 12_213       76..120 /  90..159
#     _BFGS  BierlaireQuadratic      113 /   119       11_959 / 12_776      112..229 /  93..297
...
#     _DFP   StrongWolfe(c₂=0.1)    218 /   279       18_127 / 23_828      187..868 / 198..515
#     _DFP   Bisection               136 /   111       80_001 / 65_447       99..141 / 102..124
#     _DFP   Quadratic               308 /   529       29_443 / 50_666       85..868 / 164..735
#
...
#
# What SimpleSolvers 0.12 and the step ceiling moved, separated. Both columns come from
# `svd_tables()`, the second as `svd_tables(step_ceiling = Inf)` -- the knob is a keyword on
# `solve_once` precisely so that the comparison is regenerated by the harness rather than recalled.
# With the ceiling switched off, i.e. SimpleSolvers 0.12 alone, every *pinned* figure above is
# reproduced to the digit except three, all under `Geodesic`: `_BFGS  Quadratic` 111 -> 120 iterations
# (15_377 -> 15_422 evaluations), `_BFGS  BierlaireQuadratic` 130 -> 113 (13_781 -> 11_959) and
# `_DFP   Quadratic` 175 -> 308 (18_122 -> 29_443). Every `Cayley` pinned figure is identical either
# way. So on this seed the ceiling binds only on `Geodesic` and only for the polynomial searches, and
# when it binds it is a different trajectory rather than a better or worse one -- the same thing a more
# accurate exponential or a more accurate `φ'` is, and it moves in both directions.
#
# The seed spreads move more, which is where the fix lives. The four rows that were not 8/8:
#
#                                        before          after
#     _BFGS  Quadratic     Cayley     90..cap  (4/8)   90..159  (8/8)   worst check 3.2e-1 -> 6.1e-14
#     _BFGS  Bierlaire     Cayley     93..cap  (4/8)   93..297  (8/8)   worst check 5.5e-1 -> 6.9e-14
#     _BFGS  Backtr(exp)   Geodesic  104..161  (7/8)  104..162  (8/8)   worst check 2.8e-12 -> 6.1e-14
#     _BFGS  Backtracking  Geodesic  114..192  (7/8)  146..197  (8/8)   worst check 2.8e-12 -> 6.2e-14
#
...
# all eight seeds is `2.5e-13` (`_DFP  Backtracking(expand)  Cayley`) and over the eighteen `_BFGS`
# rows it is `6.9e-14`, so `MANIFOLD_TOLERANCE` at `1e-12` clears the whole sweep.
#
# Worst `rg` over all twenty and all eight seeds is `2.5e-07`, against `3.1e-01` with the ceiling off
# -- that one being the diverging solve rather than a tolerance. See `CONVERGED_GRADIENT_TOLERANCE`.
#
...
# `2.8e-12` after it, a factor of 10^7, and is `6.1e-14` now -- per-seed `check` for that combination,
...
# exponential -- and it is not: it is one over-long step, and bounding the step takes that entry from
# `2.8e-12` to `6.1e-14` with `ScaledSquaring` untouched. The retraction was the amplifier here exactly
# as it is in A1b.
#
