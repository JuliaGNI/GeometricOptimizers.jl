...
# builds the exact Hessian, which a parameter set has no vector to carry, so `Optimizer` rejects it
# here with a message saying so. `test/optimizer_tests.jl` pins that.
