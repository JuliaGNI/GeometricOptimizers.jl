...
The difference to the standard [`Adam`](@ref) is that we change the learning reate ``\eta`` in each step.
...
"""
struct AdamWithDecay{T<:Real} <: OptimizerMethod{T}
    η₁::T
    η₂::T
    ρ₁::T
    ρ₂::T
    ...
    function AdamWithDecay(n_epochs::Int, η₁=1f-2, η₂=1f-6, ρ₁=9f-1, ρ₂=9.9f-1, δ=1f-8; T=typeof(η₁))
        γ = exp(log(η₂ / η₁) / n_epochs)
        new{T}(T(η₁), T(η₂), T(ρ₁), T(ρ₂), T(δ), T(γ), n_epochs)
    end
end

function AdamWithDecay(n_epochs::Int, T::Type; η₁=1f-2, η₂=1f-6, ρ₁=9f-1, ρ₂=9.9f-1, δ=1f-8)
    AdamWithDecay(n_epochs, T(η₁), T(η₂), T(ρ₁), T(ρ₂), T(δ))
end

function update!(o::Optimizer{<:AdamWithDecay{T}}, C::AdamCache, B::AbstractArray) where T
