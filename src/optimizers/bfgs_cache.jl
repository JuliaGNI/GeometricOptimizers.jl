...
"""
struct _BFGSCache{T, BT<:AbstractArray{T}, HT<:AbstractMatrix{T}} <: AbstractCache{T}
    B::BT
    H::HT
    function _BFGSCache(B::AbstractArray)
        zeroB = zero(B)
...
function Base.show(io::IO, ::MIME{Symbol("text/plain")}, C::_BFGSCache)
    show(io, raw"`_BFGSCache` that currently stores `B`as  ...")
...
setup_bfgs_cache(B::AbstractArray) = _BFGSCache(B)
