...
- **`optimization_step!` refuses a gradient of another element type than `x` with an `ArgumentError`** that names both types, and a gradient of another shape with a `DimensionMismatch`, before the iteration count moves. The check is dispatch: the allocation per step and the inferred return type are unchanged in both precisions (`scripts/training_step_cost.jl`).
