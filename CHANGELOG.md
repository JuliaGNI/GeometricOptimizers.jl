...
- **It reproduces GeometricMachineLearning 0.8's `optimization_step!`:** `test/training_optimizer.jl` compares 3 steps against iterates stored in `test/data/gml_reference.jl`, which `scripts/gml_reference/generate.jl` computes with GML 0.8.0 and GO 0.8.0 in their own environment (GML 0.8 cannot be a test dependency). Six models (StiefelLayer + Dense + GrassmannLayer, MultiHeadAttention(Stiefel = true) + Dense, LASympNet(4), GSympNet(4), VolumePreservingAttention(skew_sym = true), VolumePreservingLower/UpperLayer), GradientMethod / MomentumMethod / Adam, Float32 and Float64. Adam and every model without a manifold leaf match exactly (==); GradientMethod and MomentumMethod on manifold models are within 4 eps(T) relative (measured: at most 0.6 eps). The test copies the reference's random completions of the global sections into the state, because GML 0.8 draws them in a different order (a random x̄ per layer between sections) and because `global_section` orthonormalises twice since 0.8.0, which moves a completion by round-off; with the completion matched, every entry is bit-identical.
