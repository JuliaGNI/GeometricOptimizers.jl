...
- **One allocator convention for every owned array and manifold type:** `zeros([backend,] X{T}, dims::Integer...)` and `rand([rng,] [backend,] X{T}, dims::Integer...)`. A bare `X` means `default_eltype(backend)` (Float64 on CPU, Float32 on a GPU backend); backend defaults to `CPU()`, rng to `Random.default_rng()`. The chain lives in `src/allocators.jl`. The old argument order `rand(backend, rng, X, ...)` is removed and raises `MethodError`. Work that was impossible before now succeeds: `zeros(SkewSymMatrix{Float32}, Int32(3))`, `rand(SkewSymMatrix{Float32}, Int32(3))`, `zeros(CPU(), SkewSymMatrix{Float32}, Int32(3))`, `rand(CPU(), SkewSymMatrix, 3)`, `rand(CPU(), StiefelLieAlgHorMatrix, 5, 2)`, `zeros(StrictlyLowerTriangular, 3)`. A bare type on a device backend no longer hard-codes Float64: `zeros(JLBackend(), SkewSymMatrix, 3)` is Float32. `rand(rng, backend, StiefelLieAlgHorMatrix{T}, N, n)` now draws the A block before the B block on every backend, so a seeded device draw of that type gives different numbers than before; for every owned `X`, `rand(Xoshiro(1), CPU(), X{T}, dims...) == rand(Xoshiro(1), X{T}, dims...)`.
...
- **The `n == 1` branch in `SkewSymMatrix` zeros and in `map_to_Skew` is removed.** `KernelAbstractions.zeros` at length 0 works on CPU and Metal, so the special case is no longer needed.
