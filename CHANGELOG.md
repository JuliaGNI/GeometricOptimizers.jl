...
- **Allocator calls that raised a `MethodError` now return the requested type:** `zeros(SkewSymMatrix{Float32}, Int32(3))`, `rand(SkewSymMatrix{Float32}, Int32(3))`, `zeros(CPU(), SkewSymMatrix{Float32}, Int32(3))`, `rand(CPU(), SkewSymMatrix, 3)`, `rand(CPU(), StiefelLieAlgHorMatrix, 5, 2)`, `zeros(StrictlyLowerTriangular, 3)`. A bare type on a device backend takes `default_eltype(backend)` instead of a hard-coded `Float64`: `zeros(JLBackend(), SkewSymMatrix, 3)` is `Float32`.

### Changed

- **`solve!` passes the objective value it evaluates at the new iterate to the state update.** The state update `update!(state, opt, x, f)` now receives the objective value `f` that `solve!` has already evaluated at the iterate, and records it instead of evaluating the objective again. The first-order states (`GradientState`, `MomentumState`, `AdamState`, `ScalarMomentAdamState`) and the quasi-Newton states (`BFGSState`, `DFPState`) record `f` directly; other states ignore it. The three-argument form `update!(state, opt, x)` persists for callers that drive the solve loop themselves, and evaluates the objective under the `:objective` phase as before. **Effect:** for the first-order states (`GradientState`, `MomentumState`, `AdamState`, `ScalarMomentAdamState`) and the quasi-Newton states (`BFGSState`, `DFPState`), one fewer objective evaluation per iteration except the last, reducing objective event pairs by one per such iteration. With `Static(0.01)` and three iterations, the Float64 objective calls go from 10 to 8 (start, NaN-guard trial point and new iterate per step, end = 2n + 2). `NewtonState` still evaluates the objective again in its state update. The recorded values are identical, because both come from `problem(opt).F(x)` at the same `x`. The testset "solve! evaluates the objective once per iterate" covers `GradientMethod`, `MomentumMethod`, `Adam`, `BFGS`, `DFP` and `ScalarMomentAdam` on a Stiefel manifold.
...
- **The `n == 1` branch in `SkewSymMatrix` zeros and in `map_to_Skew` is removed.** `KernelAbstractions.zeros` at length 0 works on CPU and Metal, so the special case is not needed.
- **`advance_state!(state, cache, method)` is split out of the first-order state updates:** it copies `section(cache)` into the state and advances the momentum or moments. `solve!`'s `update!(state, opt, x, f)` now copies the section instead of retracting a second time; the iterates of GradientMethod, MomentumMethod and Adam on the SVD problem of test/optimizer_convergence/svd_optim.jl are bit-identical before and after, 1000 steps, Geodesic and Cayley (`scripts/svd_first_order_iterates.jl` prints a hash of the bits). The observer still reports the `:retraction_application` phase for that state update.
...
- **One allocator convention for every owned array and manifold type:** `zeros([backend,] X{T}, dims::Integer...)` and `rand([rng,] [backend,] X{T}, dims::Integer...)` (manifold types have `rand` only). A bare `X` means `default_eltype(backend)` (Float64 on CPU, Float32 on a GPU backend); backend defaults to `CPU()`, rng to `Random.default_rng()`. The chain lives in `src/allocators.jl`. The old argument order `rand(backend, rng, X, ...)` is removed and raises `MethodError`. A type that names its storage as well, such as `StrictlyLowerTriangular{Float64, Vector{Float64}}`, is a `MethodError` in every allocator; the triangular allocators accepted it before, and with a named backend returned that backend's storage in place of the one named. `rand(rng, backend, StiefelLieAlgHorMatrix{T}, N, n)` now draws the A block before the B block on every backend, so a seeded device draw of that type gives different numbers than before; for every owned `X`, `rand(Xoshiro(1), CPU(), X{T}, dims...) == rand(Xoshiro(1), X{T}, dims...)`.
- **`LowerTriangular` and `UpperTriangular` are renamed `StrictlyLowerTriangular` and `StrictlyUpperTriangular`** (they store n(n-1)/2 entries). After this change, `using GeometricOptimizers, LinearAlgebra` leaves LinearAlgebra's `LowerTriangular` usable. The NeuralNetworkParameters load registry keys change with the names ("StrictlyLowerTriangular"), so a file saved with the old type name no longer loads.
- **`NewtonOptimizerState` is renamed `NewtonState`.**
- **`gradient`, `value` and `check` are `public`, no longer exported** (Zygote exports `gradient`). Callers write `GeometricOptimizers.check` or `using GeometricOptimizers: check`.
- **`StiefelProjection` takes one argument order:** `StiefelProjection(backend, T, N, n)` and `StiefelProjection(T, N, n)`; `StiefelProjection(N, n, T)` is removed. `StiefelProjection(N, n)` (Float64, host) remains.
- **`_check_supported_eltype` is deleted.** The error type changes: an allocation of a Float64 array on Metal raised the package's `ArgumentError`; it now raises Metal's own `ErrorException("Metal does not support Float64 values, try using Float32 instead")` at allocation time. A caller that caught `ArgumentError` must catch that instead.
- **The methods and schedules carry no element type:** `Adam(; β₁, β₂, δ)`, `MomentumMethod(; α)` (keyword only; `MomentumMethod(0.5)` is a `MethodError`), `ScalarMomentAdam(; β₁, β₂, δ, ambient_norm)`, `AdamWithEuclideanDecay(; β₁, β₂, δ, λ)`, `DecayingStatic(; η₁, η₂, n)`, `AdamOptimizerWithDecay(n_epochs; η₁, η₂, kwargs...)`. `Adam(Float32)`, `DecayingStatic(Float32)`, `AdamOptimizerWithDecay(100, Float32)` etc. are `MethodError`s. `Optimizer` and `TrainingOptimizer` convert the method once with `change_precision(T, method)` to the element type of the parameters, and a `Static` or `DecayingStatic` line search too (γ computed in T). So `Optimizer(Float32[1, 2, 3], F; algorithm = Adam())` solves in Float32 (before, it threw "there is no OptimizerCache for the method Adam{Float64} and parameters of element type Float32"; `AdamWithEuclideanDecay()` threw the same). Migration: drop the type argument; `MomentumMethod(α)` becomes `MomentumMethod(; α)`. This is the GeometricOptimizers half of GeometricMachineLearning#142.
- **Deleted: the fallback `OptimizerCache(::OptimizerMethod, x)` that raised the element-type error, the ScalarMomentAdam element-type message and its three throw sites (`OptimizerState(ScalarMomentAdam(), Float32 Stiefel)` now returns a Float32 state), the `MomentumMethod` special case in `_optimizer`.** `OptimizerCache` for Adam, MomentumMethod, AdamWithEuclideanDecay and ScalarMomentAdam no longer requires the method's element type to match.
- **`update!(cache::GradientCache, state, g, ::GradientMethod, x)` replaces the `::Hessian` form,** so every first-order method is `update!(cache, state, g, method, x)`; `solver_step!` has no branch on the method any more (the Hessian-free methods are selected by dispatch; the ensure_descent! exemption of the moment methods is a method of `ensure_descent!`). The array-form `update!(state::GradientState/MomentumState/AdamState/ScalarMomentAdamState, gradient_array, direction, …, f, retraction)` methods are removed; `update!(state, opt, x)` remains.

### Added

- **`cayley(A::AbstractVecOrMat) = A` is documented as the identity by design:** the Euclidean retraction is the addition in `update_section!`. This answers GML #196.
...
- **`TrainingOptimizer(x; algorithm = Adam(), linesearch = default_step_size(algorithm), retraction = Cayley())` and `optimization_step!(x, opt, dp)` (both exported):** the training step, one step per minibatch with a caller-computed Euclidean gradient `dp` (a `NetworkParameters` of the shape of `x` for a parameter set) and a fixed or scheduled step size. `algorithm` is a first-order method (`GradientMethod`, `MomentumMethod`, `Adam`, `AdamWithEuclideanDecay`, `ScalarMomentAdam`, which steps a single `StiefelManifold` only), converted once to the element type of `x`. `linesearch` is a number (`Static(η)`), a `Static` or a `DecayingStatic`; anything else is an `ArgumentError`, because a training step has no objective to search along. The cache, state and retraction workspace cover the whole parameter set. A manual page "The Training Step" explains why this is not `solve!`.
- **`Optimizer`'s `linesearch` keyword also takes a number, which is `Static(T(η))`,** as in `TrainingOptimizer`. In both, the number has to be finite and positive; `NaN`, `Inf`, `0`, a negative number and a `Bool` raise an `ArgumentError`. The positional `Optimizer(algorithm, problem, hessian, cache, linesearch)` converts the method and the step size as the other constructors do.
- **`optimization_step!` refuses a gradient of another element type than `x` with an `ArgumentError`** that names the gradient type, the parameter type and the optimizer state type, and a gradient of another shape with a `DimensionMismatch`, before the iteration count moves. The check is dispatch: the allocation per step and the inferred return type are unchanged in both precisions (`scripts/training_step_cost.jl`).
- **`PrecomputedGradient(x, dp)` (public):** a `Gradient` holding a computed gradient; calling it on `x` returns `rgrad(x, dp)`.
- **`default_step_size(method)` (public):** `1e-2` for `GradientMethod` and `MomentumMethod`, `1e-3` for the Adam family. `step_size(::Static, t)` returns its `α`, next to `step_size(::DecayingStatic, t)`; `step_size` is public.
- **It reproduces GeometricMachineLearning 0.8's `optimization_step!`:** `test/training_optimizer.jl` compares 3 steps against iterates stored in `test/data/gml_reference.jl`, which `scripts/gml_reference/generate.jl` computes with GML 0.8.0 and GO 0.8.0 in their own environment (GML 0.8 cannot be a test dependency). Six models (StiefelLayer + Dense + GrassmannLayer, MultiHeadAttention(Stiefel = true) + Dense, LASympNet(4), GSympNet(4), VolumePreservingAttention(skew_sym = true), VolumePreservingLower/UpperLayer), GradientMethod / MomentumMethod / Adam, Float32 and Float64. The test copies the reference's random completions of the global sections into the state, because GML 0.8 draws them in a different order (a random x̄ per layer between sections) and because the second orthonormalisation of `global_section` (PR #118, after 0.8.0) moves a completion by round-off. With the completions copied, all 36 entries are bit-identical on the host they were measured on. The test asserts `==` for Adam and for the models without a manifold leaf, and a bound of 4 eps(T) relative for GradientMethod and MomentumMethod on the manifold models.
- **Cost per step, `scripts/training_step_cost.jl` against `scripts/gml_reference/step_cost.jl` (GML 0.8.0),** parameter set of StiefelLayer(64, 8) + 2 × Dense(64, 64), BLAS on one thread, minimum over 5 calls (allocation) and 200 calls (time) after two warm-up steps, three cold runs each, Apple M-series host. Allocation per step (identical in all three runs): GradientMethod 25.4 KiB vs 151.8 KiB (Float64), 13.8 vs 78.7 KiB (Float32); MomentumMethod 25.4 vs 151.9 KiB, 13.8 vs 78.8 KiB; Adam 371.5 vs 498.4 KiB, 189.1 vs 254.6 KiB. Time per step, range of three runs: GradientMethod Float64 32.2–35.9 μs vs 42.4–47.5 μs, Float32 20.8–21.0 vs 27.6–29.2; MomentumMethod Float64 36.0–40.0 vs 40.7–44.7, Float32 21.0–23.4 vs 26.7–29.2; Adam Float64 60.0–65.1 vs 61.3–65.8, Float32 37.8–43.5 vs 38.6–43.1 (Adam: no difference beyond the run-to-run spread).
...
- Added opt-in optimizer phase observation through the exported `EventLog`, `PhaseTimer`,
  ...
  implement an observer; both accept optional phase filters. The default `NoStepObserver` preserves
  the existing behavior when no observer is supplied.
...
- Added `GeometricOptimizers.default_eltype(backend)`, the element type a `rand` that names a
  backend but no element type draws in, and its docstring, which says why each of the two values is
  what it is. See *Changed*: the values are unchanged.
- Added the manual section *Where a sampled array lands, and what it holds*. `rand` and `zeros`
...
- The `rand(backend, manifold_type, N, n)` docstring now states what a backend has to supply:
  ...
  with. `CUDA` supplies it through CUSOLVER. `Metal` does not, so every such draw fails there with
  `Cannot access the contents of a private buffer`, and so do `GlobalSection(Y)` and therefore
  `Optimizer(Y, F)` — measured on real hardware. That is `Metal.jl`'s gap and not this package's,
  but a reader had no way to learn the requirement except by hitting it.
...
- **Every allocator that names a backend and an element type the backend cannot hold now refuses
  ...
  would be worse still — a call that names its element type has ruled that out, and the caller
  would carry `Float32` results through code written for `Float64` with nothing to say so.
...
- The element type a backend-taking `rand` picks when the caller names *none* is unchanged —
  ...
  with nothing stating why. `Float64` on the host is what `zeros(n)` already gives; `Float32` on a
  device is the width an accelerator is built for.

  **The rule deliberately does not ask `supports_float64`.** A backend being able to hold a
  `Float64` is not a reason to hand it one, so a device that carries `Float64` still defaults to
  `Float32`. That trait answers the other half of the question, above.
- `StiefelProjection`'s `N` and `n` are `Int` and not `Integer`. They were the package's only
  abstract fields, so `Base.return_types(size, …)` on one was `Tuple{Integer, Integer}` and every
  caller of `size` paid a dynamic dispatch for it. Nothing downstream had lost its concrete type —
  `lift_factors`, `geodesic`, `cayley` and `hcat` against a `StiefelProjection` were concrete
  already — so this is a tidy-up and not a measured speed-up.
- `StiefelProjection(N, n, T)`, `zeros(::Type{<:AbstractTriangular}, n)` and
