...
- **`Optimizer`'s `linesearch` keyword also takes a number, which is `Static(T(η))`,** as in `TrainingOptimizer`. In both, the number has to be finite and positive; `NaN`, `Inf`, `0` and a negative number raise an `ArgumentError`.
- **`optimization_step!` refuses a gradient of another element type than `x` with an `ArgumentError`** that names both types. The check is dispatch: the allocation per step and the inferred return type are unchanged in both precisions (`scripts/training_step_cost.jl`).
