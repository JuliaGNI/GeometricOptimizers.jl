...
- **`advance_state!(state, cache, method)` is split out of the first-order state updates:** it copies `section(cache)` into the state and advances the momentum or moments. `solve!`'s `update!(state, opt, x)` now copies the section instead of retracting a second time; the iterates of GradientMethod, MomentumMethod and Adam on the SVD problem of test/optimizer_convergence/svd_optim.jl are bit-identical before and after, 1000 steps, Geodesic and Cayley (`scripts/svd_first_order_iterates.jl` prints a hash of the bits). The observer still reports the `:retraction_application` phase for that state update.
