...
- **`StiefelProjection(backend, T, N, n)` with more columns than rows (`n > N`) builds on a device.** Its kernel launched `n` work items to write the diagonal, and an `N × n` matrix has `min(N, n)` diagonal entries, so the call raised a `BoundsError` — on JLArrays from the kernel, on Metal as a `KernelException`. The host constructor and a tall or square projection were not affected.
