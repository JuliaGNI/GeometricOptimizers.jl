...
We further discussed what the operations \texttt{rgrad}, \texttt{global\_rep} and ``Retraction'' look like in practice and concluded by introducing the concept of \textit{parallel transport}. The presentation was accompanied by code snippets that demonstrate the application interface of \texttt{GeometricOptimizers} throughout the chapter.
