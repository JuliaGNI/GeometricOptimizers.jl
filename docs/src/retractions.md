...
This page is the retractions themselves: what a retraction is, how the two of them are derived on a
homogeneous space, where one sits in the optimizer, and how [`Cayley`](@ref) and [`Geodesic`](@ref)
differ as maps. The numerical problem of *evaluating* the exponential — the five algorithms, what
they trade, and which one to reach for — is on the [Exponential Algorithms](@ref) page. The
optimizer that uses either retraction is described on the
[Optimization on Homogeneous Spaces](@ref) page.
...
``3\cdot10^{-13}`` over the sweep in [Staying on the manifold](@ref), which is the largest drift of
anything measured there other than [`TaylorSeries`](@ref).
